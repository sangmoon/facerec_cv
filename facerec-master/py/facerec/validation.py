...
class MyValidation(ValidationStrategy):
    ...
    def __init__(self, model):
        ...
        Args:
            model [Model] model for this validation
        """
        super(MyValidation, self).__init__(model=model)
        self.logger = logging.getLogger("facerec.validation.MyValidation")
        
    def validate(self, TX, Ty, description="ExperimentName"):
        # (X,y) = shuffle(X,y)
        # (TX, Ty) = shuffle(TX, Ty)
	Ty = np.asarray(Ty)
	...
	# self.model.compute(X, y)
        for i in range(0,n):
            ...
            # get prediction
            prediction = self.model.predict(TX[i])[0]
            if prediction == Ty[i]:
                true_positives = true_positives + 1
            else:
                false_positives = false_positives + 1
                
        self.add(ValidationResult(true_positives, 0, false_positives, 0, description))
    ...
    def __repr__(self):
        return "My Validation (model=%s)" % (self.model)
